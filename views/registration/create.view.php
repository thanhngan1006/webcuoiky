...
<div class="flex flex-col gap-5 max-w-[768px] p-5 bg-white rounded-sm border border-neutral-5 shadow overflow-hidden ">
  ...
  <div class="flex min-h-full flex-col justify-center px-6 py-12 lg:px-8">
    ...
    <form method="POST" action="/register" class="flex gap-2.5 items-end mt-6">
      ...
      <div class="flex flex-col gap-1.5">
        ...
        <input type="text" name="full_name" id="full_name" placeholder="Nhap ho va ten" class="base-input-sm" value="">
        <?php if (isset($errors['full_name'])) :  ?>
          <p class="text-red-500 text-xs mt-2"> <?= $errors['full_name'] ?></p>
        <?php endif; ?>
      </div>

      <button class="button-secondary-solid-md" type="submit" name="send">
        Create
      </button>
    </form>
