...
<div class="flex flex-col gap-4">
  <div class='flex items-center gap-1.5'>
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 512 512" stroke-width="1.5" stroke="currentColor"
      class="w-5 h-5">
      ...
      <path d="M64 127.24v257.52C64 428.52 150 464 256 464s192-35.48 192-79.24V127.24" fill="none" stroke="currentColor"
  ...
  <div class="flex flex-col gap-5 p-5 bg-white rounded-sm border border-neutral-5 shadow overflow-hidden">
      ...
      <form method="get" class="flex gap-2.5 items-end">
        <div class="flex flex-col gap-1.5">
          ...
          <input type="text" name="name" id="name" value="<?= isset($_GET['name']) ? $_GET['name'] : '' ?>"
            placeholder="Tìm tên sản phẩm" class="base-input-sm">
        </div>
        <div class="flex flex-col gap-1.5 relative">
          <label for="barcode">Mã sản phẩm</label>
          <input type="text" name="barcode" id="barcode" value="<?= isset($_GET['barcode']) ? $_GET['barcode'] : '' ?>"
            placeholder="Tìm mã sản phẩm" class="base-input-sm">
        </div>
    ...
    <div class="flex flex-col">
      <!-- PROPERTIES -->
      <div
        class="grid border-b-[2px] border-neutral-5 <?= $user['role'] === "admin" ? 'grid-cols-[1fr_1fr_1fr_1fr_1fr_1fr_1fr_1fr]' : 'grid-cols-[1fr_1fr_1fr_1fr_1fr]' ?>">
        ...
        <?php foreach ($products as $product): ?>
          ...
          <div
            ...
            <span class="p-2.5 text-neutral-10 flex items-center gap-2">
              ...
              <?php if ($user['role'] == 'admin' && $product['sold_quantity'] === 0): ?>
